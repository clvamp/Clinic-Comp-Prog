...
public class PatientMakeAppointment extends JPanel {
    ...
    private final Color HEADER_BG = new Color(245, 247, 250);     
    private final Color CARD_BG = Color.WHITE;
    private final Color TEXT_DARK = new Color(34, 40, 49);
    private final Color TEXT_MUTED = new Color(90, 90, 90);
    private final Color BUTTON_DEFAULT = new Color(22, 41, 56);   
    private final Color BUTTON_HOVER = new Color(34, 65, 87);     
    ...
    private void initializeUI() {
        ...
        if (currentUsername != null && !currentUsername.isEmpty()) {
            String formattedName = currentUsername.substring(0, 1).toUpperCase() + currentUsername.substring(1);
            patientNameField.setText(formattedName);
        }
        gbc.gridy = 1;
        ...
        // --- Row 5: Terms and Services Checkbox ---
        JPanel termsPanel = new JPanel(new FlowLayout(FlowLayout.LEFT, 0, 0));
        termsPanel.setBackground(CARD_BG);

        termsCheckBox = new JCheckBox("I have read and agree to the ");
        termsCheckBox.setFont(fieldFont);
        termsCheckBox.setBackground(CARD_BG);
        termsCheckBox.setForeground(TEXT_DARK);
        termsCheckBox.setFocusPainted(false);

        JLabel termsLinkLabel = new JLabel("Terms and Services");
        termsLinkLabel.setFont(new Font("Segoe UI", Font.BOLD, 14));
        termsLinkLabel.setForeground(COLOR_INFO); 
        termsLinkLabel.setCursor(new Cursor(Cursor.HAND_CURSOR));

        termsLinkLabel.addMouseListener(new MouseAdapter() {
            @Override
            public void mouseEntered(MouseEvent e) { termsLinkLabel.setText("<html><u>Terms and Services</u></html>"); }
            @Override
            public void mouseExited(MouseEvent e) { termsLinkLabel.setText("Terms and Services"); }
            @Override
            public void mouseClicked(MouseEvent e) {
                Window parentWindow = SwingUtilities.getWindowAncestor(PatientMakeAppointment.this);
                new TermsServices(parentWindow, () -> termsCheckBox.setSelected(true));
            }
        });
        termsPanel.add(termsCheckBox);
        termsPanel.add(termsLinkLabel);
        gbc.gridy = 10;
        gbc.insets = new Insets(10, 10, 0, 10);
        formCard.add(termsPanel, gbc);

        // --- Row 6: Action Button Panel ---
        ...
        centerConstraints.gridx = 0; centerConstraints.gridy = 0;
        centerConstraints.weightx = 1.0; centerConstraints.weighty = 1.0;
        ...
        resetBtn.addActionListener(e -> {
            ...
            if (confirm.isConfirmed()) clearFormFields();
        });
        ...
        submitBtn.addActionListener(e -> {
            ...
            if (isFormValid(name, ageText, gender, email, concern)) {
                ConfirmGUI confirm = new ConfirmGUI(parentWindow, "Submit Request", "Confirm appointment booking?", COLOR_INFO);
                if (confirm.isConfirmed()) {
                    PatientAppointmentLogic appLogic = new PatientAppointmentLogic();
                    if (appLogic.createAppointment(name, Integer.parseInt(ageText), gender, email, concern)) {
                        new NoticeGUI(parentWindow, "Success", "Appointment request sent successfully!", COLOR_SUCCESS);
                        clearFormFields();
                    } else {
                        new NoticeGUI(parentWindow, "Database Error", "Failed to submit. Check your connection.", COLOR_ERROR);
                    }
                }
            }
        });
    ...
    private boolean isFormValid(String name, String ageText, String gender, String email, String concern) {
        Window parentWindow = SwingUtilities.getWindowAncestor(this);
        if (name.isEmpty() || ageText.isEmpty() || gender.equals("Select Gender") || email.isEmpty() || concern.isEmpty()) {
            new NoticeGUI(parentWindow, "Validation Error", "Please fill out all required fields.", COLOR_ERROR);
            return false;
        }
        int age;
        try {
            ...
            new NoticeGUI(parentWindow, "Invalid Input", "Age must be a valid number.", COLOR_ERROR);
            return false;
        }
        if (age < 18) {
            new NoticeGUI(parentWindow, "Age Restriction", "You must be 18+ to book.", COLOR_ERROR);
            return false;
        }
        if (!email.contains("@")) {
            new NoticeGUI(parentWindow, "Invalid Email", "Please enter a valid email address.", COLOR_ERROR);
            return false;
        }
        if (!termsCheckBox.isSelected()) {
            new NoticeGUI(parentWindow, "Terms Required", "You must agree to the Terms and Services.", COLOR_ERROR);
            return false;
        }
        return true;
    }
